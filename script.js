...
class Ui {
    static displayproducts(e) {
        document.querySelector(".pccontainer").innerHTML += `
                  ...
                  <button class="delete" onclick="fun(${e.id})" productid=${e.id}>
                      X
                  </button>
              </div>
              `;
    ...
    static displayproductsLS() {
        ...
        document.querySelector(".pccontainer").innerHTML =s;
    }
...
document.querySelectorAll(".addtocart").forEach((btn)=>{
    btn.addEventListener("click", (element)=>{
        let pid=element.target.getAttribute(`productid`);
        console.log(pid);
        var flag=0;
        let arr=Storage.getproducts();
        console.log(arr);
        ...
        console.log(flag);
        if(flag==0){
       let obj= productslist.find((item)=> item.id==pid);
       Storage.addtolocalstorage(obj);  
...
function fun(id){
    let obj= productslist.find((item)=> item.id==id)
